...
class CDS:
    def __init__(
        self, epsilon: int = 0, num_tuples: int = 1000, input_data: str = "../data/c3o-experiments/sort.csv", dataset: str = "sort"
    ):
...
class IDS:
    def __init__(
        ...
        describer.describe_dataset_in_independent_attribute_mode(
            dataset_file=input_data, attribute_to_is_categorical=categorical_attributes, attribute_to_is_candidate_key=candidate_keys
        )
                        ...
                        dbc.Card(
                            dbc.CardBody(
                                [
                                    ...
                                    html.Br(),
                                    html.Img(id="eval_image", src="assets/no.png"),
                                ],
                        ...
                        dbc.Card(
                            dbc.CardBody(
                                [
                                    ...
                                    html.Img(id="eval_imagenn", src="assets/no.png"),
                                    html.Img(id="eval_imagenn2", src="assets/no.png"),
                                ],
                                style={"height": "100%", "display": "flex", "flex-direction": "column"},
                            )
                        ),
...
@app.callback(
    [dash.dependencies.Output("csv-table", "data"), dash.dependencies.Output("csv-table", "columns")],
    [dash.dependencies.Input("dropdown2", "value")],
    [dash.dependencies.State("dropdown2", "value")],
    prevent_initial_call=True,
)
def update_output(value, v2):
    df = pd.read_csv(f"../data/c3o-experiments/{value}.csv")
...
def update_output(n_clicks, value1, value2, value3, value4, dataset, current_src):
    ...
    if n_clicks == 0:
        ...
        ds = CDS(epsilon=value2, num_tuples=value4, input_data=f"../data/c3o-experiments/{dataset}.csv", dataset=dataset)
        df = pd.read_csv("temp/sythetic_data.csv")
        return (
            f"You selected {value1} from menu 1 and {value2}, {value3}, {value4}, {dataset} from menu 2.",
            f"assets/temp_{dataset}.png",
            n_clicks,
            df.to_dict("records"),
            [{"name": col, "id": col} for col in df.columns],
        )
    else:
        ds = IDS(epsilon=value2, num_tuples=value4, input_data=f"../data/c3o-experiments/{dataset}.csv", dataset=dataset)
        df = pd.read_csv("temp/sythetic_data.csv")
        return (
            f"You selected {value1} from menu 1 and {value2}, {value3}, {value4}, {dataset} from menu 2.",
            f"assets/temp_ids_{dataset}.png",
            n_clicks,
            df.to_dict("records"),
            [{"name": col, "id": col} for col in df.columns],
        )
...
class NN:
    def __init__(self, dataset, optimizer, split: int = 70, epochs: int = 10, n_clicks: int = 0):
        ...
        df = pd.read_csv(f"../data/c3o-experiments/{dataset}.csv")
        syndf = pd.read_csv("./temp/sythetic_data.csv")
        ...
        model = Sequential([Dense(32, input_shape=(7,)), Dense(1)])
        synmodel = Sequential([Dense(32, input_shape=(7,)), Dense(1)])
        print("Compiled standard Model")

        model.compile(optimizer=optimizer, loss="mae", metrics=["accuracy"])
        synmodel.compile(optimizer=optimizer, loss="mae", metrics=["accuracy"])

        hist = model.fit(X, Y, batch_size=32, epochs=epochs)
        hist = synmodel.fit(synX, synY, batch_size=32, epochs=epochs)
